...
location <- map(word_nodes, ~xmlGetAttr(xmlParent(.), "location")) %>% as.character()
...
corrected_pos <- read_delim("txt/Argo_added_POS.txt", delim = ";") %>%
  select(-n) %>%
  rename(word_u = word, pos_corr = pos)

Argo_dior <- Argo_dior %>% 
  mutate(word_u = betacode_to_unicode(Argo_dior$word, strict = F) )
...
##Соедененяем две таблицы
Argo_joined <- Argo_pers_tokens %>% 
  mutate(pos = Argo_dior$pos,
         word_d = Argo_dior$word)
